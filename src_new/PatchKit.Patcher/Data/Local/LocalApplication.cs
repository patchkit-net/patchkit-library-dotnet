...
namespace PatchKit.Patcher.Data.Local
{
    /// <summary>
    /// Local data.
    /// </summary>
    internal class LocalApplication
    {
        ...
        /// </summary>
        public bool CheckFilesConsistency(int contentVersion, AppContentSummary contentSummary, CancellationToken cancellationToken)
        {
            foreach (var summaryFile in contentSummary.Files)
            {
                ...
                if (Cache.GetFileVersion(summaryFile.Path) != contentVersion)
                {
                    return false;
                }
